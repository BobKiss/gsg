<?php
$post_id = get_the_ID();
if (isset($hourly_range['min_hours']) && !empty($hourly_range['min_hours'])) {
    $min_hours = $hourly_range['min_hours'];
} else {
    $min_hours = '';
}

if (isset($hourly_range['max_hours']) && !empty($hourly_range['max_hours'])) {
    $max_hours = $hourly_range['max_hours'];
} else {
    $max_hours = '';
}

if (isset($hourly_range['range_cost']) && !empty($hourly_range['range_cost'])) {
    $range_cost = $hourly_range['range_cost'];
} else {
    $range_cost = '';
}

if (isset($hourly_range['cost_applicable']) && !empty($hourly_range['cost_applicable'])) {
    $cost_applicable = $hourly_range['cost_applicable'];
} else {
    $cost_applicable = '';
}
...
<div class="hourly_range_group redq-remove-rows sort ui-state-default" style="background: none; border: none;">
    ...
    <div class="redq-hide-row" style="margin: 15px;">
        ...
        $custom_attr_min = $pricing_type === 'flat_hours' ? array('step' => 1, 'min' => 1,) : array('step' => 1, 'min' => 0, 'max' => 24);
