...
class WC_Redq_Rental_Cart
{
    ...
    function calculate_inventory_data()
    {
        $posted = $_POST['form'];
        ...
        $quantity = (isset($posted_data['quantity']) && !empty($posted_data['quantity'])) ? $posted_data['quantity'] : 1;
        $final_cost = wc_price($posted_data['rental_days_and_costs']['cost'] * $quantity);

        $response_data = array(
            'available_quantity' => $posted_data['available_quantity'],
    ...
    public function get_posted_data($product_id, $posted_data)
    {
        ...
        $inventory_id = $posted_data['booking_inventory'];
    ...
    public function rnb_data_validation($product_id, $request_data)
    {
        ...
        $labels = reddq_rental_get_settings($product_id, 'labels', array('notice'));
        $conditional_data = reddq_rental_get_settings($product_id, 'conditions');
        ...
        if ($request_data['pickup_date'] && $request_data['dropoff_date']) {
            ...
            if (!empty($conditional_data['min_book_days']) && $rental_duration['days'] < $conditional_data['min_book_days']) {
                $response['error'][] = sprintf(__('%s is %s', 'redq-rental'), $labels['min_day_notice'], $conditional_data['min_book_days']);
            }

            if (!empty($conditional_data['max_book_days']) && $rental_duration['days'] > $conditional_data['max_book_days']) {
                $response['error'][] = sprintf(__('%s is %s', 'redq-rental'), $labels['max_day_notice'], $conditional_data['max_book_days']);
            }
        }

        //available quantity
        if ($request_data['selected_qty'] > $request_data['available_qty']) {
            $response['error'][] = sprintf(__('%s %s', 'redq-rental'), $request_data['selected_qty'], $labels['quantity_notice']);
        }
    ...
    public function calculate_cost($product_id, $inventory_id, $data, $conditional_data)
    {
        ...
        switch ($pricing_type) {
            case 'flat_hours':
                $cost = $this->calculate_flat_hours_pricing_plan_cost($booking_args, $product_id, $inventory_id, $data, $conditional_data);
                break;
            case 'general_pricing':
                $cost = $this->calculate_general_pricing_plan_cost($booking_args, $product_id, $inventory_id, $data, $conditional_data);
                break;
            case 'daily_pricing':
                $cost = $this->calculate_daily_pricing_plan_cost($booking_args, $product_id, $inventory_id, $data, $conditional_data);
                break;
            case 'monthly_pricing':
                $cost = $this->calculate_monthly_pricing_plan_cost($booking_args, $product_id, $inventory_id, $data, $conditional_data);
                break;
            case 'days_range':
                $cost = $this->calculate_day_ranges_pricing_plan_cost($booking_args, $product_id, $inventory_id, $data, $conditional_data);
                break;
        }
    ...
    public function calculate_rental_days($data, $conditional_data)
    {
        ...
        if ($hours < 24) {
            ...
            $days = $conditional_data['pay_extra_hours'] !== 'yes' && $extra_hours > floatval($max_hours_late) ? $days + 1 : $days;
    ...
    public function calculate_flat_hours_pricing_plan_cost($booking_args, $product_id, $inventory_id, $data, $conditional_data)
    {
        ...
        $cost = $this->calculate_hourly_extras_cost($cost, $total_hours, $payable_cat, $payable_resource, $payable_person, $payable_security_deposites, $pickup_cost, $dropoff_cost, $location_cost, true, $product_id, $inventory_id, $data, $conditional_data);
    ...
    public function calculate_general_pricing_plan_cost($booking_args, $product_id, $inventory_id, $data, $conditional_data)
    {
        ...
        if ($days > 0) {
            ...
            $day_cost = $this->calculate_extras_cost($day_cost, $rental_days, $payable_cat, $payable_resource, $payable_person, $payable_security_deposites, $pickup_cost, $dropoff_cost, $location_cost, $product_id, $inventory_id, $data, $conditional_data);

            if ($extra_hours_payment === 'yes' && $extra_hours > 0) {
                $hour_cost = $this->calculate_hourly_price($extra_hours, $pricing_data);
                $hour_cost = $this->calculate_hourly_extras_cost($hour_cost, $extra_hours, $payable_cat, $payable_resource, $payable_person, $payable_security_deposites, $pickup_cost, $dropoff_cost, $location_cost, false, $product_id, $inventory_id, $data, $conditional_data);
            }

            $cost = $day_cost + $hour_cost;
        } else {
            $cost = $this->calculate_hourly_price($hours, $pricing_data);
            $cost = $this->calculate_hourly_extras_cost($cost, $hours, $payable_cat, $payable_resource, $payable_person, $payable_security_deposites, $pickup_cost, $dropoff_cost, $location_cost, true, $product_id, $inventory_id, $data, $conditional_data);
        }
    ...
    public function calculate_day_ranges_pricing_plan_cost($booking_args, $product_id, $inventory_id, $data, $conditional_data)
    {
        ...
        if ($days > 0) {
            ...
            $day_cost = $this->calculate_price_discount($day_cost, $price_discount, $rental_days);
            $day_cost = $this->calculate_extras_cost($day_cost, $rental_days, $payable_cat, $payable_resource, $payable_person, $payable_security_deposites, $pickup_cost, $dropoff_cost, $location_cost, false, $product_id, $inventory_id, $data, $conditional_data);
