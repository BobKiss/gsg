...
if (!empty($redq_product_inventory))
    $categories = $product->redq_get_rental_payable_attributes('rnb_categories', $redq_product_inventory[0]);
?>
            ...
            <div class="attributes categories-attr">
                <label class="custom-block">
                    ...
                    <input type="checkbox" name="categories[]"
                           value="<?php echo esc_attr($value['name']); ?>|<?php echo esc_attr($value['cost']); ?>|<?php echo esc_attr($value['applicable']); ?>|<?php echo esc_attr($value['hourlycost']); ?>|1"
                           data-name="<?php echo esc_attr($value['name']); ?>" data-value-in="0"
                           data-applicable="<?php echo esc_attr($value['applicable']); ?>"
                           data-value="<?php echo esc_attr($value['cost']); ?>"
                           data-hourlyrate="<?php echo esc_attr($value['hourlycost']); ?>" data-currency-before="$"
                           data-currency-after="" class="carrental_categories">
                    <?php echo esc_attr($value['name']); ?>

                    <?php if ($value['applicable'] == 'per_day') { ?>
                        <span class="pull-right show_if_day"><?php echo wc_price($value['cost']); ?><span><?php _e(' - Per Day', 'redq-rental'); ?></span></span>
