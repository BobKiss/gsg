...
$extra_labels = reddq_rental_get_settings($product_id, 'layout_two', array('location', 'datetime', 'resource', 'person', 'deposit', 'summary'));
$extra_datetime_labels = $extra_labels['datetime'];
$extra_location_labels = $extra_labels['location'];

$labels = reddq_rental_get_settings($product_id, 'labels', array('inventory'));
$labels = $labels['labels'];

$redq_product_inventory = rnb_get_product_inventory_id($product_id);

if (empty($redq_product_inventory)) {
    wc_add_notice(sprintf(__('You don\'t have any invertory with this product. This product in not bookable.', 'redq-rental')));
    return;
}
            ...
            <div id="rnbSmartwizard">
                ...
                <section class="rnb-step-content-wrapper">
                    <div id="rnb-step-1" class="">
                        <div class="payable-inventory rnb-component-wrapper rnb-select-wrapper redq-pick-up-location" <?php echo (count($inventories) < 2) ? 'style="display:none"' : ''; ?>>
                            <h5><?php echo esc_attr($labels['inventory']); ?></h5>
                            <select class="rnb-select-box" id="booking_inventory" name="booking_inventory"
                        ...
                        <div class="rnb-google-location-map-area">
                            <div class="rnb-autocomplete-input-group">
                                <input type="text" name="pickup_location" class="rnb-pickup-location"
                                       id="rnb-origin-autocomplete"/>
                                <input type="text" name="dropoff_location" class="rnb-dropoff-location"
                                       id="rnb-destination-autocomplete"/>
                            </div>
                    ...
                    <div id="rnb-step-2" class="">
                        ...
                        <?php if (isset($displays['quantity']) && $displays['quantity'] !== 'closed') : ?>
                            <div class="redq-quantity rnb-select-wrapper rnb-component-wrapper">
                                ...
                                <input type="number" name="inventory_quantity" class="inventory-qty" min="" max=""
                                       value="1">
                            </div>
                        <?php endif; ?>
                    </div>
                        ...
                        <div id="rnb-step-3" class="">
                            ...
                            <div class="rnb-resource-checkbox-area">
                                <?php foreach ($resources as $key => $value) { ?>
                                    <label class="rnb-control rnb-control-checkbox"
                                           for="rnb-resource-<?php echo $key + 1; ?>">
                                        ...
                                        <?php } ?>
                                        <input type="checkbox" data-items="<?php echo $key + 1; ?>"
                                               id="rnb-resource-<?php echo $key + 1; ?>" name="extras[]"
                                               ...
                                               data-name="<?php echo esc_attr($value['resource_name']); ?>"
                                               data-value-in="0"
                                               ...
                                               data-currency-before="$" data-currency-after=""
                                               class="carrental_extras"/>
                                    </label>
                                <?php } ?>
                            </div>
                        </div>
                            ...
                            <div class="rnb-persons-section">
                                ...
                                <div class="person-radiobtns-area rnb-adult-area">
                                    <?php foreach ($adults as $key => $value) { ?>
                                        <label class="rnb-control rnb-control-radio rnb-adult-label"
                                               for="rnb-adult-<?php echo $key + 1; ?>">
                                            ...
                                            <?php } ?>
                                            <input class="additional_adults_info" type="radio"
                                                   name="additional_adults_info" data-items="<?php echo $key + 1; ?>"
                                                   id="rnb-adult-<?php echo $key + 1; ?>"
                                                   value="<?php echo esc_attr($value['person_count']); ?>|<?php echo esc_attr($value['person_cost']); ?>|<?php echo esc_attr($value['person_cost_applicable']); ?>|<?php echo esc_attr($value['person_hourly_cost']); ?>"
                                                   data-person_cost="<?php echo esc_attr($value['person_cost']); ?>"
                                                   data-person_count="<?php echo esc_attr($value['person_count']); ?>"
                                                   data-applicable="<?php echo esc_attr($value['person_cost_applicable']); ?>"/>
                                        </label>
                                    <?php } ?>
                                </div>
                                ...
                                <div class="person-radiobtns-area rnb-child-area">
                                    <?php foreach ($childs as $key => $value) { ?>
                                        <label class="rnb-control rnb-control-radio rnb-child-label"
                                               for="rnb-child-<?php echo $key + 1; ?>">
                                            <span class="title"><?php echo esc_attr($value['person_count']); ?></span>
                                            <?php if ($value['person_cost_applicable'] == 'per_day') { ?>
                                                <span class="meta show_if_day"><?php echo wc_price($value['person_cost']); ?><?php _e(' - Per Day', 'redq-rental'); ?></span>
                                                <span class="meta show_if_time"><?php echo wc_price($value['person_hourly_cost']); ?><?php _e(' - Per Hour', 'redq-rental'); ?></span>
                                            <?php } else { ?>
                                                <span class="meta"><?php echo wc_price($value['person_cost']); ?><?php _e(' - One Time', 'redq-rental'); ?></span>
                                            <?php } ?>
                                            <input type="radio" class="additional_childs_info"
                                                   name="additional_childs_info" data-items="<?php echo $key + 1; ?>"
                                                   id="rnb-child-<?php echo $key + 1; ?>"
                                                   value="<?php echo esc_attr($value['person_count']); ?>|<?php echo esc_attr($value['person_cost']); ?>|<?php echo esc_attr($value['person_cost_applicable']); ?>|<?php echo esc_attr($value['person_hourly_cost']); ?>"
                                                   data-person_cost="<?php echo esc_attr($value['person_cost']); ?>"
                                                   data-person_count="<?php echo esc_attr($value['person_count']); ?>"
                                                   data-applicable="<?php echo esc_attr($value['person_cost_applicable']); ?>"/>
                            ...
                            <div class="rnb-deposite-section">
                                <div class="deposite-checkbox-area">
                                    <?php foreach ($security_deposites as $key => $value) { ?>
                                        <label class="rnb-control rnb-control-checkbox rnb-deposit-label"
                                               for="rnb-deposit-<?php echo $key + 1; ?>">
                                            ...
                                            <?php if ($value['security_deposite_applicable'] == 'per_day') { ?>
                                                ...
                                                <span class="meta show_if_time"
                                                      style="display: none;"><?php echo wc_price($value['security_deposite_hourly_cost']); ?><?php _e(' - Per Hour', 'redq-rental'); ?></span>
                                            <?php } else { ?>
                                                <span class="meta"><?php echo wc_price($value['security_deposite_cost']); ?><?php _e(' - One Time', 'redq-rental'); ?></span>
                                            <?php } ?>
                                            <input type="checkbox" data-items="<?php echo $key + 1; ?>"
                                                   id="rnb-deposit-<?php echo $key + 1; ?>" <?php if ($value['security_deposite_clickable'] === 'no') { ?> checked onclick="return false" <?php } ?>
                                                   name="security_deposites[]"
                                                   value="<?php echo esc_attr($value['security_deposite_name']); ?>|<?php echo esc_attr($value['security_deposite_cost']); ?>|<?php echo esc_attr($value['security_deposite_applicable']); ?>|<?php echo esc_attr($value['security_deposite_hourly_cost']); ?>"
                                                   data-name="<?php echo esc_attr($value['security_deposite_name']); ?>"
                                                   data-value-in="0"
                                                   data-applicable="<?php echo esc_attr($value['security_deposite_applicable']); ?>"
                                                   data-value="<?php echo esc_attr($value['security_deposite_cost']); ?>"
                                                   data-hourly-rate="<?php echo esc_attr($value['security_deposite_hourly_cost']); ?>"
                                                   data-currency-before="$" data-currency-after=""/>
                                        </label>
                                    <?php } ?>
                                </div>
